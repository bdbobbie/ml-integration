...
final class ML_IntegrationTests: XCTestCase {
    ...
    @MainActor
    func testConcurrencyLimitPersistsAcrossViewModelRelaunch() async throws {
        let envKey = RuntimeEnvironment.testRootEnvironmentVariable
        let previous = getenv(envKey).map { String(cString: $0) }
        let testRoot = FileManager.default.temporaryDirectory
            .appendingPathComponent("ml-integration-concurrency-persist-\(UUID().uuidString)", isDirectory: true)
        ...
        first.updateMaxConcurrentRunningVMs(3)
        XCTAssertEqual(first.maxConcurrentRunningVMs, 3)
        ...
        XCTAssertEqual(second.maxConcurrentRunningVMs, 3)
    }

    @MainActor
    func testConcurrencyLimitDefaultsWhenNoSettingsFileExists() async throws {
        let envKey = RuntimeEnvironment.testRootEnvironmentVariable
        let previous = getenv(envKey).map { String(cString: $0) }
        let testRoot = FileManager.default.temporaryDirectory
            .appendingPathComponent("ml-integration-concurrency-default-\(UUID().uuidString)", isDirectory: true)
        ...
        XCTAssertEqual(viewModel.maxConcurrentRunningVMs, 2)
    }
    ...
    @MainActor
    func testQueuedStartAutoRunsWhenBlockingVMStops() async throws {
        let installerURL = try makeTemporaryInstallerImage()
        defer { try? FileManager.default.removeItem(at: installerURL) }
    ...
    @MainActor
    func testQueuedStartAutoRunsAfterIncreasingConcurrencyLimit() async throws {
        let installerURL = try makeTemporaryInstallerImage()
        defer { try? FileManager.default.removeItem(at: installerURL) }
    ...
    @MainActor
    func testQueuedStartFailureSchedulesRetryAndKeepsVMQueued() async throws {
        let installerURL = try makeTemporaryInstallerImage()
        defer { try? FileManager.default.removeItem(at: installerURL) }
        ...
        XCTAssertEqual(viewModel.queuedStartVMIDs, [vmID])
        XCTAssertEqual(viewModel.queuedStartRetryCounts[vmID], 1)
        XCTAssertNotNil(viewModel.queuedStartNextAttemptAtByVMID[vmID])
        XCTAssertEqual(viewModel.runtimeState(for: vmID), .failed)
    }

    @MainActor
    func testQueuedStartDropsAfterMaxRetries() async throws {
        let installerURL = try makeTemporaryInstallerImage()
        defer { try? FileManager.default.removeItem(at: installerURL) }
        ...
        viewModel.enqueueManagedVMStart(vmID)
        await viewModel.runQueuedStartSchedulerTick() // attempt 1
        try? await Task.sleep(nanoseconds: 1_100_000_000)
        await viewModel.runQueuedStartSchedulerTick() // attempt 2
        try? await Task.sleep(nanoseconds: 1_100_000_000)
        await viewModel.runQueuedStartSchedulerTick() // attempt 3 => drop

        XCTAssertTrue(viewModel.queuedStartVMIDs.isEmpty)
        XCTAssertNil(viewModel.queuedStartRetryCounts[vmID])
        XCTAssertNil(viewModel.queuedStartNextAttemptAtByVMID[vmID])
        XCTAssertTrue(viewModel.vmRuntimeStatusMessage.contains("Dropped queued start"))
    }
    ...
    @MainActor
    func testClearQueuedStartsRemovesQueueAndRetryMetadata() async throws {
        let installerURL = try makeTemporaryInstallerImage()
        defer { try? FileManager.default.removeItem(at: installerURL) }
        ...
        viewModel.clearQueuedStarts()
        XCTAssertTrue(viewModel.queuedStartVMIDs.isEmpty)
        XCTAssertTrue(viewModel.queuedStartRetryCounts.isEmpty)
        XCTAssertTrue(viewModel.queuedStartNextAttemptAtByVMID.isEmpty)
    }

    @MainActor
    func testResetQueuedStartRetriesClearsCooldownAndResetsCounts() async throws {
        let installerURL = try makeTemporaryInstallerImage()
        defer { try? FileManager.default.removeItem(at: installerURL) }
        ...
        XCTAssertEqual(viewModel.queuedStartRetryCounts[vmID], 0)
        XCTAssertNil(viewModel.queuedStartNextAttemptAtByVMID[vmID])
    }
    ...
    @MainActor
    func testFormattedLauncherRunHistoryReturnsEmptyForNoHistory() {
        let viewModel = RuntimeWorkbenchViewModel(
            hostService: MockHostService(),
            catalogService: MockCatalogService(),
            provisioningService: MockProvisioningService(),
        ...
        let text = viewModel.formattedLauncherRunHistory(vmID: UUID(), limit: 5)
        XCTAssertEqual(text, "")
    }

    @MainActor
    func testFormattedLauncherRunHistoryReturnsJoinedLines() async throws {
        let envKey = RuntimeEnvironment.testRootEnvironmentVariable
        let previous = getenv(envKey).map { String(cString: $0) }
        let testRoot = FileManager.default.temporaryDirectory
            .appendingPathComponent("ml-integration-launcher-format-history-\(UUID().uuidString)", isDirectory: true)
        setenv(envKey, testRoot.path, 1)
        defer {
            if let previous { setenv(envKey, previous, 1) } else { unsetenv(envKey) }
            try? FileManager.default.removeItem(at: testRoot)
        ...
        await viewModel.scaffoldInstall(
            ...
            architecture: .appleSilicon,
            runtime: .appleVirtualization,
            vmName: "vm-launcher-format-history",
            installerImagePath: installerURL.path,
            kernelImagePath: "",
            initialRamdiskPath: ""
        )
        ...
        let text = viewModel.formattedLauncherRunHistory(vmID: vmID, limit: 5)
        XCTAssertFalse(text.isEmpty)
        XCTAssertTrue(text.contains(launcherEntry.name))
        XCTAssertTrue(text.contains("Succeeded"))
    }

    @MainActor
    func testFormattedFilteredLauncherRunHistoryReturnsEmptyWhenNoMatches() async throws {
        let envKey = RuntimeEnvironment.testRootEnvironmentVariable
        let previous = getenv(envKey).map { String(cString: $0) }
        let testRoot = FileManager.default.temporaryDirectory
            .appendingPathComponent("ml-integration-launcher-format-filter-empty-\(UUID().uuidString)", isDirectory: true)
        setenv(envKey, testRoot.path, 1)
        defer {
            if let previous { setenv(envKey, previous, 1) } else { unsetenv(envKey) }
            try? FileManager.default.removeItem(at: testRoot)
        ...
        await viewModel.scaffoldInstall(
            ...
            architecture: .appleSilicon,
            runtime: .appleVirtualization,
            vmName: "vm-launcher-format-filter-empty",
            installerImagePath: installerURL.path,
            kernelImagePath: "",
            initialRamdiskPath: ""
        )
        ...
        let text = viewModel.formattedFilteredLauncherRunHistory(
            vmID: vmID,
            statusFilter: .failed,
            searchTerm: "no-hit",
            limit: 5
    ...
    @MainActor
    func testFormattedFilteredLauncherRunHistoryReturnsMatchingLines() async throws {
        let envKey = RuntimeEnvironment.testRootEnvironmentVariable
        let previous = getenv(envKey).map { String(cString: $0) }
        let testRoot = FileManager.default.temporaryDirectory
            .appendingPathComponent("ml-integration-launcher-format-filter-match-\(UUID().uuidString)", isDirectory: true)
        setenv(envKey, testRoot.path, 1)
        defer {
            if let previous { setenv(envKey, previous, 1) } else { unsetenv(envKey) }
            try? FileManager.default.removeItem(at: testRoot)
        ...
        await viewModel.scaffoldInstall(
            ...
            architecture: .appleSilicon,
            runtime: .appleVirtualization,
            vmName: "vm-launcher-format-filter-match",
            installerImagePath: installerURL.path,
            kernelImagePath: "",
            initialRamdiskPath: ""
        )
    ...
    @MainActor
    func testFleetLauncherRunHistoryJSONReturnsEmptyWhenNoHistory() {
        let viewModel = RuntimeWorkbenchViewModel(
            hostService: MockHostService(),
            catalogService: MockCatalogService(),
            provisioningService: MockProvisioningService(),
        ...
        let json = viewModel.fleetLauncherRunHistoryJSON(filteredBy: .all, limitPerVM: 10)
        XCTAssertEqual(json, "")
    }

    @MainActor
    func testFleetLauncherRunHistoryJSONReturnsPayloadForFleetFilter() async throws {
        let envKey = RuntimeEnvironment.testRootEnvironmentVariable
        let previous = getenv(envKey).map { String(cString: $0) }
        let testRoot = FileManager.default.temporaryDirectory
            .appendingPathComponent("ml-integration-fleet-launcher-json-\(UUID().uuidString)", isDirectory: true)
        setenv(envKey, testRoot.path, 1)
        defer {
            if let previous { setenv(envKey, previous, 1) } else { unsetenv(envKey) }
            try? FileManager.default.removeItem(at: testRoot)
        ...
        let allJSON = viewModel.fleetLauncherRunHistoryJSON(filteredBy: .all, limitPerVM: 10)
        let runningJSON = viewModel.fleetLauncherRunHistoryJSON(filteredBy: .running, limitPerVM: 10)
        XCTAssertFalse(allJSON.isEmpty)
        XCTAssertTrue(allJSON.contains(vmID.uuidString))
        XCTAssertTrue(allJSON.contains(launcherEntry.name))
        XCTAssertTrue(runningJSON.isEmpty)
    }
